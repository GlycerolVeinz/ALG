 ...
 * @return tile on coord, nullptr otherwise
 * */
...
vector<Tile *> getAllNeighbours(Garden *garden, Tile *tile) {
    ...
    for (std::pair<int, int> neighbourDelta: ALL_NEIGHBOURS) {
        Tile *neighbour = getNeighbourTile(garden, tile, neighbourDelta);
        if (neighbour)
...
Garden *readInput() {
    ...
    for (size_t y = 0; y < garden->height; ++y) {
        ...
        garden->gardenMap.at(RIGHT_DIR).at(y).resize(garden->width);
        for (size_t x = 0; x < garden->width; ++x) {
